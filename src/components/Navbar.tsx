  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 
      ...
      <div className="festival-container py-4 flex justify-between items-center bg-[#080829]/[0.87]">
        ...
        <div className="hidden md:flex items-center space-x-6">
          {navLinks.map(link => link.submenu ? <div key={link.id} className="relative group">
                ...
                <div className="absolute top-full left-0 mt-1 py-2 backdrop-blur-md rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-[200px] bg-indigo-950">
        ...
        <div className={cn("absolute top-full left-0 right-0 bg-festival-primary/90 backdrop-blur-md shadow-md transition-all duration-300 md:hidden", isMenuOpen ? "max-h-[400px] opacity-100" : "max-h-0 opacity-0 overflow-hidden")}>
          <div className="flex flex-col p-4 space-y-4 bg-[#050F2C]/95">
            ...
            {navLinks.map(link => link.submenu ? <div key={link.id} className="space-y-2">
                  <div className="text-white/80 font-medium pl-2">{link.title}</div>
                  <div className="pl-4 space-y-2">
                    {link.submenu.map(subItem => <Link key={subItem.id} to={subItem.url} className="block text-white/70 hover:text-white" onClick={toggleMenu}>
                        {subItem.title}
                      </Link>)}
                  </div>
                </div> : <Link key={link.id} to={link.url} className="nav-link text-white hover:text-white" onClick={toggleMenu}>
                  {link.title}
                </Link>)}
